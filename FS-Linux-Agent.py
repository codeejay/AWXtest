...
AccountURI = "https://dynatraceservicedesk.freshservice.com"
ProbeKey = ""
RegistrationKey="eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJwb3J0YWxfdXJsIjoiaHR0cHM6Ly9keW5hdHJhY2VzZXJ2aWNlZGVzay5mcmVzaHNlcnZpY2UuY29tIn0.6-aBrAUR2mtadu50fzJHhE3nzu6sZUDzS_pkVIosFV4"
ProxyServer = ""
ProxyUserName = ""
ProxyPassword = ""
ProxyPort = ""
...
account_domain = urlparse.urlparse(AccountURI) if is_python2 else urlparse(AccountURI)
...
manifest = urllib2.urlopen(manifest_url).read() if is_python2 else urlopen(manifest_url).read()
import json
...
u = urllib2.urlopen(url) if is_python2 else urlopen(url)
...
try:
    ...
    print(e)
execute_shell_command_ex_handled("tar -zxvf " + file_name+" -C " + curr_dir)
...
execute_shell_command_ex_handled("/bin/bash " + os.path.join(curr_dir, "Freshservice-Discovery-Agent", "setup.sh"))
try:
    import shutil
